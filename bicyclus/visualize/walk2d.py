...
def plot(args):
    ...
    if args.x is not None and args.y is not None:
        if args.x not in summary.index or args.y not in summary.index:
            msg = (f"Could not find keys {(args.x, args.y)}. "
        ...
        try:
            ...
            print(f"ERROR: Could not find keys ('posterior', {args.x}) and "
...
def main():
    p = argparse.ArgumentParser(
        description='visualize a 2D walk along distribution')
    p.add_argument('input_cdf',
                   type=str,
                   default=None,
    ...
    p.add_argument('--x',
                   type=str,
                   default=None,
                   help='Parameter space: x dimension')
    p.add_argument('--y',
                   type=str,
                   default=None,
                   help='Parameter space: y dimension')
    p.add_argument('-l',
                   action='store_const',
                   const=True,
                   default=False,
    ...
    p.add_argument('--first',
                   type=int,
                   default=-1,
                   help='Only consider first X samples')
    p.add_argument('--hist',
                   action='store_const',
                   const=True,
                   default=False,
