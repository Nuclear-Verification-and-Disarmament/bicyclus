...
def plot_samples(trace_samples,
                 draw_samples,
                 plotfile,
                 densfile,
                 combined=True):
...
def run(args):
    ...
    if not args.json:
        ...
        log.log_print('Available posterior distributions:',
                      list(trace_samples.posterior))
        print(trace_samples.posterior)
        print(az.summary(trace_samples))

        plot_samples(trace_samples if args.dim == 'chain' else draw_samples,
                     draw_samples,
                     update_filename(args.trace_plot_file.format(format=args.format), args.jobid),
                     update_filename(args.density_plot_file.format(format=args.format),
                                     args.jobid), args.combined)
        ...
        if len(draw_samples.posterior) == 2 or args.hist_vars is not None:
            plot_hist2d(args.hist_vars.split(',') if args.hist_vars else [],
                        draw_samples,
                        update_filename(args.histogram_plot_file.format(format=args.format), args.jobid),
                        kind=args.hist_kind)
        if args.outcdf:
            trace_samples.to_netcdf(update_filename(args.outcdf, args.jobid))
    else:  # args.json
        if not files:
            files = enumerate_files_for_jobid(args.trace_dir,
                                              args.jobid,
                                              typ='json')
        traces = [read_json_file_to_trace(f) for f in files]
        merged = az.concat(traces, dim='draw')
        log.log_print('Available posterior distributions:',
                      list(merged.posterior))
        log.log_print('Using JSON traces: merging along `draw`')
        print(merged.posterior)
        print(az.summary(merged))
        plot_samples(merged, merged,
                     update_filename(args.trace_plot_file.format(format=args.format), args.jobid),
                     update_filename(args.density_plot_file.format(format=args.format), args.jobid),
                     args.combined)
        if args.outcdf:
...
def main():
    ...
    p.add_argument(
        ...
        default=-1,
        help=
        'Job ID whose traces to process. This expects CDF files of the pattern: <job name stuff>_<job id>_<task id>_<iteration>.cdf'
    )
    p.add_argument('--json',
                   default=False,
                   action='store_true',
                   help='Analyse samples from JSON logs instead of CDF files.')
    p.add_argument('--trace-plot-file',
                   type=str,
                   default='plot_merge_trace.{format}',
                   help='Plot output for trace plot')
    p.add_argument('--density-plot-file',
                   type=str,
    ...
    p.add_argument(
        ...
        default='kde',
        help=
        'Type of 2D histogram if selected. One of mpl (for matplotlib hist2d), kde/scatter/hist/hex/reg/resid'
    )
    p.add_argument('--format', type=str, choices=('pdf', 'png'), default='png', help='Output format for plots')
    p.add_argument('--combined',
                   action='store_true',
                   help='Combine results of all chains in one graph line')
    p.add_argument('--no-combined', dest='combined',
                   action='store_false',
                   help='Combine results of all chains in one graph line')
    p.add_argument('--outcdf',
                   type=str,
                   default='merged.cdf',
    ...
    p.add_argument(
        ...
        default=None,
        help=
        'If there are more than two variables available, select which ones to use for plotting a 2D histogram. E.g., "enrichment,cycle_time"'
    )
    p.add_argument(
        '--dim',
        type=str,
        default='chain',
        help=
        'Whether to merge along `chain` or `draw` (https://arviz-devs.github.io/arviz/api/generated/arviz.concat.html) for the trace plot.'
    )
    args = p.parse_args()
